...
export default async function DashboardPage() {
  ...
  if (!orgMembership) {
    // Fallback: query by agent_id directly if org setup failed
    const fallbackOrgId = null;
    const { data: packages } = await supabase
      .from("packages")
      .select("id, name, token, client_name, status, dropbox_folder_url, last_viewed_at, created_at, package_talents(id, talent_id, client_pick, client_comment, client_status, client_rating, media_requested, upload_status, talents(full_name, photo_url))")
      .eq("agent_id", user.id)
    ...
    const normalized = (packages ?? []).map((p) => ({
      ...
      package_talents: p.package_talents.map(
        (pt: { id: string; talent_id: string; client_pick: boolean; client_comment: string | null; client_status: string | null; client_rating: number | null; media_requested: boolean; upload_status: string; talents: { full_name: string; photo_url: string | null } | { full_name: string; photo_url: string | null }[] }) => ({
          ...pt,
    ...
    const stats = {
      ...
      pendingMedia: allPkgTalents.filter((pt) => pt.media_requested && pt.upload_status === "pending").length,
      uploadsReceived: allPkgTalents.filter((pt) => pt.upload_status === "uploaded").length,
    };
    return <DashboardClient packages={normalized as Parameters<typeof DashboardClient>[0]["packages"]} stats={stats} />;
  }
